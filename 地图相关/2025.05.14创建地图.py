...
def create_graph_from_files(edges_walk_file, edges_bike_file, nodes_file):
    G = nx.Graph()
    # 读取节点文件
    ...
    # 读取边文件
    with open(edges_walk_file, 'r') as f:
        for line in f:
            source, target = map(int, line.strip().split(','))
            G.add_edge(source, target, vehicle='walk')

    with open(edges_bike_file, 'r') as f:
        for line in f:
            source, target = map(int, line.strip().split(','))
            if G.has_edge(source, target):
                G.edges[(source, target)]['vehicle'] = 'both'
            else:
                G.add_edge(source, target, vehicle='bike')
...
# 使用真实数据文件创建图
edges_walk_file = 'edges走人.txt'
edges_bike_file = 'edges走车.txt'
nodes_file = 'nodes.txt'
G = create_graph_from_files(edges_walk_file, edges_bike_file, nodes_file)
...
import pickle
with open(f'graphMaps\\{'{:06d}'.format(1)}.pickle', 'wb') as f:
    pickle.dump(g, f)
